...
rules <-  apriori(Transaction, 
                  parameter = list(supp = a(),conf=b(), minlen = 2), appearance = list(default="lhs",rhs= d()))
...
df.cus <- data.frame(value.support, value.confidence) %>% na.omit
...
visNetwork(nodes, edges, height = "100%", width = "100%",
           main = paste("Frequent patterns for", d(), 
                                  " - ", no.cus, "potential customers")) %>% visEdges(arrows = "to") %>% 
